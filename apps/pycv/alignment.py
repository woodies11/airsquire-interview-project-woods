...
def _fft_band_shift(a, b):
    (dx, _), _ = cv2.phaseCorrelate(a, b)
    return dx

def fft_shift_candidates(img1, img2, bands=3, extra_jitter=2, band_min_h=64):
    ...
    for (y0, y1) in ys:
        ...
        wc = win_col[None, :]
        win = wr * wc
        dx = _fft_band_shift(a * win, b * win)
...
def hill_climb_ssim(obj: SSIMObjective, start, init_step=256, min_step=2, patience=2):
    W = obj.W
    x = int(start) % W
...
def micro_refine(obj: SSIMObjective, center, span=12):
    W = obj.W
...
def best_yaw_shift_hillclimb(ref_img, input_img,
    ...
    # --- FFT seeds ---
    seeds, _ = fft_shift_candidates(img1_small, img2_small,
    ...
    def solve_from(seed):
        x, s = hill_climb_ssim(obj, start=seed,
                               init_step=init_step, min_step=min_step)
        x_ref, s_ref = micro_refine(obj, x, span=micro_span)
        return x_ref, s_ref
